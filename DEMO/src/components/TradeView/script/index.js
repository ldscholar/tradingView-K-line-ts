...
export default {
  ...
  getBars: function (symbol, resolution, from, to, callback) {
    let data;
    const symbolData = this.dataCache[symbol];//拿缓存的数据
    ...
    const fetchCacheData = data => {
      ...
      if (count > 0) {
        ...
        callback && callback({s: 'ok', bars: newBars})//把缓存数据传给 回调函数
      } else {
      ...
      Io.subscribeKline(params, this.onUpdateData.bind(this));//请求新数据，并更新缓存数据
    };
    // 请求数据
  ...
  onUpdateData: function (data) {
    ...
    if (data.kline.length) {
      data.kline.forEach(elm => {
        this.dataCache[data.symbol][data.resolution].push(elm)//更新 dataCache: {}, // 缓存数据
      })
    }
  }
}
